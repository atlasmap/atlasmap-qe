...
Feature: mappings table
  ...
  Scenario: check combine mapping in table and check if table is filled properly
    ...
    Then check that row number "0" contains "/sourceInteger,/sourceFloat,/sourceString" as sources, "/targetString" as target and "Many to One (Space [ ])" as type
  ...
  Scenario: check separate mapping in table and check if table is filled properly
    ...
    Then check that row number "0" contains "/sourceCombineString" as sources, "/targetString,/targetInteger,/targetFloat" as target and "One to Many (Space [ ])" as type
